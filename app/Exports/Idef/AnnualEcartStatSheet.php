...
/**
 * Feuille annuelle Écart PAX / Go-Pass (granularité mensuelle).
 * Même principe : D = B - C via formule Excel.
 */
class AnnualEcartStatSheet implements WithTitle, ShouldAutoSize, FromArray, WithEvents
{
    ...
    public function array(): array
    {
        ...
        $data[] = ['MOIS', 'NOMBRE PAX EMBARQUES', 'NOMBRE DES GO-PASS RAMASSES', 'ECART', 'JUSTIFICATIONS'];

        foreach ($this->rows as $row) {
            $monthLabel  = $this->getMonthName($row['MOIS'] ?? '');
            $totalPax    = 0;
            $totalGopass = 0;
            $rowCopy     = $row;
            array_shift($rowCopy);
        ...
        $data[] = ['TOTAL', '', '', '', $this->getAnnualJustifications()];

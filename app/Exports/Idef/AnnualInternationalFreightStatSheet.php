...
/**
 * Feuille annuelle Fret International — même structure que la mensuelle
 * mais granularité mensuelle (MOIS au lieu de DATE).
 *
 * Colonnes F..J : toutes des formules Excel (mêmes règles).
 */
class AnnualInternationalFreightStatSheet implements WithTitle, ShouldAutoSize, FromArray, WithEvents
{
    ...
    public function array(): array
    {
        ...
        foreach ($this->rows as $monthlyData) {
            [$arrTrafic, $depTrafic, $arrIdef, $depIdef] = $this->getRawFreightTotals($monthlyData);
            $data[] = [
                $this->getMonthName($monthlyData['MOIS']), // A
                $arrTrafic,  // B
                $depTrafic,  // C
                $arrIdef,    // D
                $depIdef,    // E
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                for ($row = $firstDataRow; $row <= $lastDataRow - 1; $row++) {
                    $s->getCell("F{$row}")->setValue("=B{$row}-D{$row}");
                    $s->getCell("G{$row}")->setValue("=C{$row}-E{$row}");
                    $s->getCell("H{$row}")->setValue("=D{$row}*0.007");
                    $s->getCell("I{$row}")->setValue("=E{$row}*0.005");
                    $s->getCell("J{$row}")->setValue("=H{$row}+I{$row}");
    ...
    private function getRawFreightTotals(array $monthlyData): array
    {
        $depTrafic = 0;
        $depIdef = 0;
        $arrTrafic = 0;

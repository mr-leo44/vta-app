...
class AnnualInternationalFreightStatSheet implements WithTitle, ShouldAutoSize, FromArray, WithEvents
{
    ...
    {
        $cols = 10; // MOIS + TOTAL FRET (DEB/EMB) + TOTAL FRET IDEF (DEB/EMB) + ECART (EXONERE) (DEB/EMB) + PERCEPTION ESTIMEE HORS DGDA (DEB/EMB + TOTAL)
        $data = [];
        ...
        // EN-TÊTES
        $data[] = ['MOIS', 'TOTAL FRET', '', 'TOTAL FRET IDEF', '', 'ECART (EXONERE)', '', 'PERCEPTION ESTIMEE HORS DGDA', '', '']; // En-têtes des colonnes
    ...
    private function getFreightOrExecedentDatas(array $rows): array
    {
        ...
        foreach ($rows as $key => $monthlyData) {
            $month = [$this->getMonthName($monthlyData['MOIS']) ?? ''];
            array_shift($monthlyData); // Remove the first row which contains the mois

            $valueDatas = $this->getFreightDatas($monthlyData);
            
            $datas[] = array_merge($month, $valueDatas);
        }
        return $datas;
    }

    private function getFreightDatas(array $monthlyData): array
    {

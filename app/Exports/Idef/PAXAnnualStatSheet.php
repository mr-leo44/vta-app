...
class PAXAnnualStatSheet implements FromArray, ShouldAutoSize, WithTitle, WithEvents
{
    ...
    public function array(): array
    {
        ...
        // ✅ DONNÉES : On ne met QUE les mois, le reste sera écrit dans AfterSheet
        foreach ($this->rows as $row) {
            $dataRow = [$this->getMonthName($row['MOIS']) ?? ''];
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $totalColIndex = count($this->operators) + 2; // MOIS + operators + TOTAL
                ...
                for ($row = $firstDataRow; $row <= $lastDataRow; $row++) {
                    ...
                    $colIndex = 2; // Commence à la colonne B (après MOIS)
                    foreach ($this->getCommercialOperators($this->operators) as $op) {

...
class PaxbusWeeklyInternationalSheet implements FromArray, ShouldAutoSize, WithEvents, WithTitle
{
    ...
    {
        $cols = 4; // DATE | COMPAGNIES | IMMATRICULATION | PAX
        $arrayData = [];
        ...
        // EN-TÊTES
        $arrayData[] = ['DATE', 'COMPAGNIES', 'IMMATRICULATION', 'PAX'];
        ...
        foreach ($this->data['data'] as $date => $operators) {
            ...
            foreach ($operators as $operatorSigle => $flights) {
                foreach ($flights as $flight) {
                    $row = [
                        $isFirstOperatorForDate ? $date : '', // Date uniquement sur la première ligne
                        $operatorSigle,
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                for ($row = $headerRow; $row <= $lastDataRow; $row++) {
                    ...
                    if ($row >= $firstDataRow) {
                        // Forcer les 0 dans la colonne PAX
                        $paxValue = $s->getCell("D{$row}")->getValue();

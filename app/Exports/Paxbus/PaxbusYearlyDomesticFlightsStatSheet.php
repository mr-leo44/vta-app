...
class PaxbusYearlyDomesticFlightsStatSheet implements FromArray, ShouldAutoSize, WithTitle, WithEvents
{
    ...
    public function array(): array
    {
        ...
        foreach ($this->rows['pax'] as $row) {
            ...
            foreach ($this->operators as $op) {
                $flightsCount = 0;
                foreach ($row[$op] as $opAircrafts) {
                    $flightsCount += $opAircrafts['count'];
                }
                $dataRow[] = $flightsCount ?? 0;
            }
